...
@Model(adaptables = SlingHttpServletRequest.class, adapters = DestinationDefaultPromotions.class, defaultInjectionStrategy = DefaultInjectionStrategy.OPTIONAL)
public class DestinationDefaultPromotions {
    ...
    public List<Promotions> getPropertiesList() throws IOException {
        ...
        JsonNode propertiesNodesArray = jsonResponse.path("data").path("jcr").path("defaultPromotions").path("nodes");

        logger.info("JSON received from JsonNode {}", propertiesNodesArray);
        List<Promotions> propertiesList = mapper.readValue(propertiesNodesArray.toString(),
                new TypeReference<List<Promotions>>() {
                });

        logger.info("DestinationDefaultPromotions {}", propertiesList);
    ...
    public static List<Promotions> filterEditorialPromotions(
        ...
        return promotions.stream()
                ...
                .filter(p -> p.getName().toLowerCase().contains("editorial-"))
